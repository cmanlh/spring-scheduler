...
public abstract class Task implements Runnable, ScheduledFuture<Object> {
    ...
    private void _run(Map<String, Object> param) {
        ...
        try {
            ...
            if (null != monitor) {
                monitor.notificate(startTaskEvent.setHappendTime(actualExecutionTime).setTaskId(this.id).setType(TaskEventType.START)
                                .setParam(_param));
            }
        ...
        if (null != monitor) {
            monitor.notificate(new TaskEvent().setHappendTime(new Date()).setTaskId(this.id)
                            ...
                            .setFailPrintList(failPrintList).setNextExecutionTime(nextExecutionTime).setStartTime(startTaskEvent.getHappendTime())
