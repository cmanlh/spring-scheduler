  ...
  private void _run(Map<String, Object> param) {
    ...
    try {
      Map<String, Object> _param =
          null != param ? param : (null == this.param ? new WeakHashMap<String, Object>() : new WeakHashMap<String, Object>(this.param));
      ...
      if (null != monitor) {
        ...
        new Thread(new Runnable() {
          ...
          @Override
          public void run() {
            monitor.notificate(startTaskEvent.setHappendTime(_actualExecutionTime).setTaskId(taskId).setType(TaskEventType.START).setParam(_param));
          }
        }).start();
      }
      ...
      new Thread(new Runnable() {
        ...
        public void run() {
          monitor.notificate(new TaskEvent().setHappendTime(new Date()).setTaskId(taskId)
              .setType(TaskStatusEnum.COMPLETED == taskStatus ? TaskEventType.COMPELETE : TaskEventType.FAIL).setFailPrintList(_failPrintList)
              .setNextExecutionTime(_nextExecutionTime).setStartTime(startTaskEvent.getHappendTime()).setParam(startTaskEvent.getParam()));
        }
      }).start();
    }
